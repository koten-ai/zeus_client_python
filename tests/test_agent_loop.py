...
async def test_setup_turn_context(patched_loop):
    tc = await loop_mod.setup_turn_context(
        "http://zeus", {"auth_mode": "none"}, "v2", "analytics",
        "beer-sample", "_default", "hello", [], False, "grok",
        "https://api.x.ai/v1", "conv-1",
    )
...
async def test_setup_turn_context_business_logic_and_toon(patched_loop, monkeypatch):
    ...
    tc = await loop_mod.setup_turn_context(
        "http://zeus", {}, "v2", "auto", "b", "s", "q", [], True, "grok", "http://llm", None,
    )
...
async def test_setup_turn_context_hash_exception(patched_loop, monkeypatch):
    ...
    tc = await loop_mod.setup_turn_context(
        "http://zeus", {}, "v1", "auto", "b", "s", "q", [], False, "grok", "http://llm", "c",
    )
    assert tc.stamped_h == ""
...
async def test_setup_turn_context_cache_hints(patched_loop, monkeypatch):
    ...
    tc = await loop_mod.setup_turn_context(
        "http://zeus", {}, "v2", "auto", "b", "s", "q", [], False, "grok", "http://llm", "c",
    )
...
async def test_run_agent_prior_turns_delta(patched_loop, monkeypatch):
    ...
    async def capture_commit(zeus_url, sid, enable_sessions, this_user_round, contract_id,
                             contract_hash, chat_req, produced_delta, prior_turns,
                             this_turn_reqs, trace, zeus_headers):
        captured["delta"] = produced_delta
        return {"session_id": sid, "round": 1, "enabled": False,
                "contract_id": "", "contract_hash": "", "contract_status": "none", "req_ids": []}
