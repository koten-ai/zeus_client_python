...
async def test_create_zeus_session_success_200(http_client):
    route = respx.post(f"{ZEUS_URL}/v1/session").mock(return_value=httpx.Response(
        200,
    ...
    assert url == f"{ZEUS_URL}/v1/session"
...
async def test_create_zeus_session_success_201(http_client):
    respx.post(f"{ZEUS_URL}/v1/session").mock(return_value=httpx.Response(201, json={"session_id": SID}))
...
async def test_create_zeus_session_non_dict_json_body(http_client):
    respx.post(f"{ZEUS_URL}/v1/session").mock(return_value=httpx.Response(200, json=[1, 2]))
    status, body, _, _ = await create_zeus_session(
...
async def test_create_zeus_session_invalid_json_on_success(http_client):
    respx.post(f"{ZEUS_URL}/v1/session").mock(return_value=httpx.Response(200, text="not-json"))
    status, body, _, _ = await create_zeus_session(
        ZEUS_URL, "b", "s", "", "", {}, [], HEADERS,
    )
    assert status == 200
...
async def test_create_zeus_session_non_2xx(http_client):
    respx.post(f"{ZEUS_URL}/v1/session").mock(return_value=httpx.Response(409, text="conflict"))
...
async def test_create_zeus_session_http_error(http_client):
    respx.post(f"{ZEUS_URL}/v1/session").mock(side_effect=httpx.ConnectError("down"))
    status, body, url, req_id = await create_zeus_session(
        ZEUS_URL, "b", "s", "", "", {}, [], HEADERS,
    )
    assert status == 0
...
async def test_continue_session_turn_success(http_client):
    route = respx.post(f"{ZEUS_URL}/v1/session/{SID}/turn").mock(return_value=httpx.Response(
    ...
    assert url.endswith(f"/v1/session/{SID}/turn")
...
async def test_continue_session_turn_invalid_json(http_client):
    respx.post(f"{ZEUS_URL}/v1/session/{SID}/turn").mock(return_value=httpx.Response(200, text="nope"))
    status, body, _, _ = await continue_session_turn(ZEUS_URL, SID, 2, {}, [], HEADERS)
    assert status == 200
...
async def test_continue_session_turn_http_error(http_client):
    respx.post(f"{ZEUS_URL}/v1/session/{SID}/turn").mock(side_effect=httpx.ReadTimeout("slow"))
...
async def test_post_session_trace_success(http_client):
    route = respx.post(f"{ZEUS_URL}/v1/session/trace").mock(return_value=httpx.Response(
    ...
    assert url == f"{ZEUS_URL}/v1/session/trace"
...
async def test_post_session_trace_non_success(http_client):
    respx.post(f"{ZEUS_URL}/v1/session/trace").mock(return_value=httpx.Response(500, text="fail"))
    status, body, _, _ = await post_session_trace(
...
async def test_post_session_trace_invalid_json(http_client):
    respx.post(f"{ZEUS_URL}/v1/session/trace").mock(return_value=httpx.Response(200, text="raw"))
    status, body, _, _ = await post_session_trace(
...
async def test_post_session_trace_http_error(http_client):
    respx.post(f"{ZEUS_URL}/v1/session/trace").mock(side_effect=httpx.ConnectError("x"))
    status, body, url, req_id = await post_session_trace(
        ZEUS_URL, SID, 1, "", "", "", {}, [], {}, "ok", HEADERS,
    )
    assert status == 0
...
async def test_rehydrate_session_success(http_client):
    ...
    respx.get(f"{ZEUS_URL}/v1/session/{SID}?rounds=6").mock(
...
async def test_rehydrate_session_invalid_json(http_client):
    respx.get(f"{ZEUS_URL}/v1/session/{SID}?rounds=4").mock(
