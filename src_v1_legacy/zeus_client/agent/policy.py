"""Post-terminate Client policy table (base-5 law).

ZC-WISH-010/013/014/020 — model triggers are signals; Client policy is law.
"""
...
from zeus_client.agent.layer_a import LayerA, artifacts_view, ui_view
from zeus_client.agent.settings import ClientSettings

# Default policy_action values
...
def map_message(
    ...
    if policy == POLICY_REFUSE:
        ...
        # Prefer model summary if it already soft-refused and has content
...
def decide_policy(
    ...
    *,
    ...
    if hooks_must_refuse:
        ...
        POLICY_ANSWER, POLICY_CLARIFY, POLICY_REFUSE, POLICY_ERROR,
    ):
