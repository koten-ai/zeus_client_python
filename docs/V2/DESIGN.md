...
**Primary mandate:** First-class integration with the Zeus Engine Debug Tool (Detective / session timeline / TraceBundle)
...
Zeus Engine (server) owns graph storage, FTS/ANN, TraceBundle per HTTP hop, and Hub Detective. The client owns everything that never hits Zeus as a single in-process ReAct turn — especially the **LLM** and the **multi-hop glue**.
...
1. **One HTTP hop → one `req_id` → one TraceBundle.** No parent merge id on the server.
...
1. Server-side multi-hop TraceBundle merge (eliminates identical multi-POST join).  
